...
dados_manifesto <- dados_manifesto[dados_manifesto$study_ID == "GSE13507", c("sample_ID", "sample_type", "characteristics")]
...
ids <- intersect(rownames(norm_corrigido_GSE13507), rownames(dados_manifesto))
...
fator_GSE13507 <- factor(dados_manifesto$sample_type,levels = c("non_tumor", "tumor"))
...
all(rownames(norm_corrigido_GSE13507) == rownames(dados_manifesto))

# arquivo para metafor
write.csv(metafor_GSE13507, "metafor_GSE13507.csv", row.names = TRUE)
