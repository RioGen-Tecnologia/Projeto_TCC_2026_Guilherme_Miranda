...
n_tumor <- sum(matriz_modelo_GSE3167[, "tumor"])
n_nontumor <- sum(matriz_modelo_GSE3167[, "non_tumor"])
...
rownames(metafor_GSE3167) <- rownames(fit2$coefficients)
...
saveRDS(
  metafor_GSE3167,
  file = file.path(out_dir, "metafor_GSE3167.rds")
)
...
deg_GSE3167 <- deg_GSE3167[!is.na(deg_GSE3167$ENTREZID), ]

# opcional: ordenar por FDR
deg_GSE3167 <- deg_GSE3167[order(deg_GSE3167$FDR), ]

# salvar
write.csv(
  deg_GSE3167,
  file = file.path(results_dir, "DEGs_tables", "DEGs_GSE3167.csv"),
  row.names = FALSE
)
