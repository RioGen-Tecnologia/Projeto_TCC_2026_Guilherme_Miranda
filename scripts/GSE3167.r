...
dados_manifesto <- dados_manifesto[dados_manifesto$study_ID == "GSE3167", c("sample_ID", "sample_type", "characteristics")]
...
cels.GSE3167 <- list.files(pattern = "[cC][eE][lL]\\.gz$")
...
ids <- intersect(rownames(norm_corrigido_GSE3167), rownames(dados_manifesto))
...
fator_GSE3167 <- factor(dados_manifesto$sample_type,levels = c("non_tumor", "tumor"))
...
all(rownames(norm_corrigido_GSE3167) == rownames(dados_manifesto))

# arquivo para metafor
write.csv(metafor_GSE3167, "metafor_GSE3167.csv", row.names = TRUE)
